...
const formData = ref<WorkOrderDisposalApi.WorkOrderDisposal>();
const getTitle = computed(() => {
  return formData.value?.id
    ? $t('ui.actionTitle.edit', [$t('work_order.disposal.name')])
    : $t('ui.actionTitle.create', [$t('work_order.disposal.name')]);
});
...
const [Modal, modalApi] = useVbenModal({
  ...
  onOpenChange(isOpen) {
    if (isOpen) {
      const data = modalApi.getData<WorkOrderDisposalApi.WorkOrderDisposal>();
      if (data) {
        formData.value = data;
        formApi.setValues(formData.value);
      }

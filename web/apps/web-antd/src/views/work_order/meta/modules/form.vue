...
const formData = ref<WorkOrderMetaApi.WorkOrderMeta>();
const getTitle = computed(() => {
  return formData.value?.id
    ? $t('ui.actionTitle.edit', [$t('work_order.meta.name')])
    : $t('ui.actionTitle.create', [$t('work_order.meta.name')]);
});
...
const [Modal, modalApi] = useVbenModal({
  ...
  onOpenChange(isOpen) {
    if (isOpen) {
      const data = modalApi.getData<WorkOrderMetaApi.WorkOrderMeta>();
      if (data) {
        formData.value = data;
        formApi.setValues(formData.value);
      }
